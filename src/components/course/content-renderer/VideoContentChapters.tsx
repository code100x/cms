        ...
        <div className="flex w-full flex-wrap gap-2">
          {(segments as Segment[])?.map(({ start, end, title }, index) => {
            return (
              ...
              >
                <div
                  className={`flex max-w-64 cursor-pointer items-center justify-between gap-2 rounded-lg px-4 py-2 text-xs transition-all duration-300 hover:bg-primary/10 md:text-sm ${currentTime >= start && currentTime < end ? 'bg-blue-500/10' : 'bg-primary/5'}`}
                  onClick={() => {
