...
const isYouTubeSrc = (src: string) => {
  const youtubePattern =
    /(?:youtu|youtube)(?:\.com|\.be)\/(?:watch\?v=)?([\w-]+)/;
  return youtubePattern.test(src);
};
const extractYouTubeId = (src: string) => {
      ...
      {source.type === 'video/youtube' ? (
        <div className="aspect-video w-full">
          <iframe
            ...
            src={`https://www.youtube.com/embed/${extractYouTubeId(source.src)}?autoplay=1&controls=1`}
