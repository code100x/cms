...
import { Content, Folder, Video, getCourse, getCurrentContentType, getFullCourseContent, getNextVideo } from '@/db/course';
...
function findContentById(contents: (Folder | Video | Content)[], ids: number[]) {
...
export default async function Course({ params }: { params: { courseId: string[] } }) {
  ...
  const fullCourseContent: Folder[] = await getFullCourseContent(parseInt(courseId))
  const courseContent = findContentById(fullCourseContent, rest.map(x => parseInt(x)))
  const contentType = courseContent?.length === 1 ? courseContent[0]?.type : "folder"
  ...
  return <div className="justify-between mx-auto text-white">
    <CourseView rest={rest} course={course} contentType={contentType} nextContent={nextContent} courseContent={courseContent} fullCourseContent={fullCourseContent} />
  </div>
}
